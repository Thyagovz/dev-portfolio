<script lang="ts">
  ...
  let { image, class: className = "", ...restProps }: Props = $props();

  let component = $state<HTMLElement | null>(null);
...
<div class={clsx("relative h-full w-full", className)} bind:this={component}>
  <div
    class="avatar aspect-square overflow-hidden rounded-3xl border-2 border-slate-700 opacity-0"
  >
    <PrismicImage
      field={image}
